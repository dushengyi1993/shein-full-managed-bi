...
test('home names every loading group and offers an explicit cache refresh', async () => {
  ...
  assert.match(home, /商家账单、补款与扣款/);
  assert.match(home, /库存台账数量与金额/);
  assert.match(home, /主销地区、趋势与排行榜/);
...
test('monthly trend uses source-aware sales and non-additive stock boundaries', async () => {
  ...
  assert.match(monthly, /definition\.aggregate === 'first'/);
...
test('approved homepage uses six paired cards and keeps operating, finance and ledger facts independent', async () => {
  ...
  assert.match(rankings, /storeAmountMetric/);
  assert.match(rankings, /storeQuantityMetric/);
  assert.match(rankings, /'netDealAmount'/);
  assert.match(rankings, /'salesQuantity'/);
  assert.match(rankings, /'estimatedDealAmount'/);
  assert.match(rankings, /'goodsCount'/);
