...
test('static assets are versioned together at 20260730.9', async () => {
  const html = await read('src/web/index.html');
  for (const asset of ['app.js', 'styles.css', 'home-parity.css', 'favicon.svg']) {
    assert.match(html, new RegExp(`/${asset.replace('.', '\\.')}\\?v=20260730\\.9`), asset);
  }
  assert.doesNotMatch(html, /\?v=20260730\.8/);
});
