...
export const BACKUP_ROOT = '/srv/shein-fm/backups/db';
const RUNTIME = '/srv/shein-fm/runtime/backup-nas-sync';
const KEY = '/srv/shein-fm/secrets/backup-nas-sync/id_ed25519';
const HOSTS = '/srv/shein-fm/secrets/backup-nas-sync/known_hosts';
const NAME = /^shein-fm-(weekly|deploy)-(\d{8}T\d{6}Z)\.dump$/;
const MAX_BYTES = 100 * 1024 ** 3;
const fail = (code) => { throw Object.assign(new Error(code), { code }); };

export function compareBackupNames(a, b) {
  const matchA = typeof a === 'string' ? a.match(NAME) : null;
  const matchB = typeof b === 'string' ? b.match(NAME) : null;
...
export function validateSourceName(source, root = BACKUP_ROOT) {
  if (typeof source !== 'string' || path.dirname(source) !== root
      || !NAME.test(path.basename(source))) fail('SOURCE_PATH_INVALID');
  return source;
}
...
export async function openSnapshot(source, root = BACKUP_ROOT) {
  validateSourceName(source, root);
  ...
  try {
    ...
    return { handle, stream, hash, assertUnchanged, name: path.basename(source), bytes: Number(st.size) };
  } catch (error) {
    await handle.close();
    throw error;
  }
...
// Never forward subprocess stderr or thrown process objects into receipts.
export async function transferChild(executable, args, snapshot, header = null, spawnImpl = spawn) {
  ...
  try {
    ...
    try { await pipeline(snapshot.stream(), child.stdin); }
      ...
      // pg_restore --list may successfully exit after reading only the TOC.
      // Never grant this early-EOF exception to the SSH archive transfer.
...
async function main() {
  ...
  const names = args.latest ? (await fs.readdir(BACKUP_ROOT)).filter(n => NAME.test(n)).sort(compareBackupNames) : [];
  if (args.latest && !names.length) fail('NO_COMPLETED_BACKUP');
  const source = args.latest ? path.join(BACKUP_ROOT, names.at(-1)) : args.source;
  let lock;
  let snapshot;
  try {
    if (args.execute) {
    ...
    snapshot = await openSnapshot(source);
    ...
    await transferChild('/usr/bin/docker', ['exec', '-i', 'shein-fm-db', 'pg_restore', '--list'], snapshot);
    const reply = await transferChild('/usr/bin/ssh', [
      ...
      'dushengyi@192.168.1.59',
      '/usr/bin/python3 /home/dushengyi/.local/libexec/shein-fm-backup-receiver.py',
    ], snapshot, manifest);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch(error => {
    const allowed = /^(ARGUMENT_INVALID|SOURCE_REQUIRED|SOURCE_PATH_INVALID|SOURCE_ROOT_SYMLINK|SOURCE_INVALID|ARCHIVE_MAGIC_INVALID|SOURCE_CHANGED|TRANSFER_FAILED|CHILD_FAILED|RECEIPT_INVALID|RECEIPT_MISMATCH|NO_COMPLETED_BACKUP|SSH_FILE_INVALID|SOURCE_CHANGED_AFTER_TRANSFER)$/;
