...
function loginPage() {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>全托店铺云端登录</title><style>
  ...
  *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.5 system-ui,-apple-system,"Segoe UI",sans-serif}header{padding:28px 32px;border-bottom:1px solid var(--line);background:#fff}h1{margin:0;font-size:25px}.sub{color:var(--muted);margin-top:6px}.wrap{max-width:1180px;margin:auto;padding:28px}.summary{display:flex;gap:14px;margin-bottom:18px}.pill{background:#fff;border:1px solid var(--line);border-radius:14px;padding:12px 16px}.pill b{font-size:20px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.store{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px}.row{display:flex;justify-content:space-between;gap:12px;align-items:center}.code{font-size:18px;font-weight:760}.status{font-size:12px;color:var(--muted)}button{border:0;border-radius:10px;padding:9px 13px;background:var(--black);color:#fff;font-weight:700;cursor:pointer}button[disabled]{opacity:.35;cursor:not-allowed}.done{color:var(--green)}.bad{color:var(--red)}#message{position:sticky;top:10px;z-index:3;background:#171717;color:#fff;border-radius:13px;padding:12px 16px;margin-bottom:16px;display:none}@media(max-width:800px){.grid{grid-template-columns:1fr}.wrap{padding:18px}}
  ...
  let activeUrl=sessionStorage.getItem('fmStoreLoginActiveUrl')||'';function openActive(){if(activeUrl)window.open(activeUrl,'_blank','noopener,noreferrer');else msg('请先关闭当前会话再重新打开。')}
  async function start(storeCode){try{const j=await api('/api/store-login/start',{method:'POST',body:JSON.stringify({storeCode})});activeUrl=j.openUrl;sessionStorage.setItem('fmStoreLoginActiveUrl',activeUrl);window.open(activeUrl,'_blank','noopener,noreferrer');await refresh()}catch(e){msg('打开失败：'+e.message)}}
  async function finish(storeCode){try{const j=await api('/api/store-login/finish',{method:'POST',body:JSON.stringify({storeCode})});activeUrl='';sessionStorage.removeItem('fmStoreLoginActiveUrl');msg(j.verified?'登录验证成功，可以处理下一家。':'未能确认店铺身份，请重新打开并检查登录账号。');await refresh()}catch(e){msg('验证失败：'+e.message);await refresh()}}
  async function closeActive(){try{await api('/api/store-login/close',{method:'POST',body:'{}'});activeUrl='';sessionStorage.removeItem('fmStoreLoginActiveUrl');await refresh()}catch(e){msg('关闭失败：'+e.message)}}
  if(!TOKEN)msg('链接无效：缺少一次性口令。');else refresh().catch(e=>msg('无法读取状态：'+e.message));
  </script></body></html>`;
}

function sessionPage(id) {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>云端 Chrome</title><style>body{margin:0;background:#111;color:#fff;font:14px system-ui}.bar{height:48px;padding:0 16px;display:flex;align-items:center;justify-content:space-between;background:#171717}iframe{display:block;width:100vw;height:calc(100vh - 48px);border:0}</style></head><body><div class="bar"><b>全托店铺云端 Chrome</b><span>登录并保存密码后，回到店铺列表点“登录完成并验证”</span></div><iframe id="frame" allow="clipboard-read; clipboard-write"></iframe><script>const t=new URLSearchParams(location.hash.slice(1)).get('token')||'';history.replaceState(null,'',location.pathname);document.getElementById('frame').src='/store-login/novnc/vnc.html?autoconnect=1&resize=scale&path='+encodeURIComponent('api/store-login/ws/${encodeURIComponent(id)}/'+encodeURIComponent(t));</script></body></html>`;
}
...
async function handle(req, res) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/store-login/start') {
    ...
    if (state.active) return json(res, 409, { ok: false, error: `请先完成 ${state.active.storeCode}` });
