...
7. **BI 服务**：只读 JSON，不持有数据库或平台凭据，不把缺数转换为零。核心文件按文件版本缓存在内存，首页使用带日期、负责人/店铺和关键词参数的 `/api/home` 按需读取，只返回当前区间、等长对比区间及金额/销量 Top 货号并集；大响应协商 gzip。所有员工读取同一份全店数据；负责人/店铺仅作筛选，不限制看数范围。
