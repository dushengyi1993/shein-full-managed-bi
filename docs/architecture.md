...
flowchart LR
    ...
    M --> SO["发货订单 shipping-orders.json"]
    J --> BI["只读 Portal / API"]
    H --> BI
    SO --> BI
```
...
5. **Webhook 接收、Worker 与定向回查**：公网接收器只验签、限流并持久化密文；Worker 才能解密和标准化。Worker 仍不持有 OpenAPI 凭据，只写回查指令和固定唤醒标记；独立的供应链最小权限进程只对已实现适配器的采购单、交付单执行精确业务键 OpenAPI 回查，入仓验真后才完成指令。标准化事件 30 秒内合并唤醒 Dashboard，回查另有 10 分钟兜底。空队列不能证明进程健康，Receiver 和 Worker 都必须有新鲜心跳。
6. **Dashboard 物化器**：使用独立只读数据库角色，读取销量、供应链、商品身份、权限与运行健康。轻量核心写入 `dashboard.json`，首页历史写入 `dashboard.home.json`，发货订单全量检索索引写入 `shipping-orders.json`；所有 staging 文件成功后才按固定顺序原子提升，避免 Portal 观察到半套新版本。
7. **订单管理物化器**：与 Dashboard 物化器同一原子提升链路产出 `order-management.json`。固定八页合同（发货单、备货记录、运单、退货申请、退货列表、收货/退货异常、增值服务列表、质检报告）中，“发货单”与“运单”核心只从 `fact.purchase_order / fact.delivery` 单次可重复读快照物化；其余六页在加密会话快照存在时按已验证的 `sso.geiwohuo.com` 固定 POST 合同合并（`/idms/order-apply/list`、`/clms/waybill/page`、`/clms/waybill/statistics`、`/pfmp/returnPlan/list`、`/pfmp/returnOrder/page`、`/pfmp/exceptionWorkorder/order/page`、`/vssv/order/page`、`/gmpj/quality/qcReportNew`）。发货台因实时待办且无总数接口已整体移除，不占位；行内 `metrics / facts / details` 只含字段哈希校验过的脱敏 allowlist 文本/数字，地址、联系人、电话、图片与报告 URL 不落盘。分页/总数/去重/内容完整性/25 店覆盖任一失败，索引 `coverage` 非 `COMPLETE` 且 `promotable=false`，提升脚本只保留 staging 文件，不发布新索引。
8. **BI 服务**：只读 JSON，不持有数据库或平台凭据，不把缺数转换为零。核心文件按文件版本缓存在内存，首页使用带日期、负责人/店铺和关键词参数的 `/api/home` 按需读取，只返回当前区间、等长对比区间及金额/销量 Top 标准货号并集；同一 owner-confirmed 标准货号的跨店成员行完整保留，未归并商品仍以店内身份隔离。大响应协商 gzip。认证用户可通过 `refresh=1` 丢弃 Portal 进程内解析缓存并重新读取已物化文件；该操作不触发 SHEIN 抓取、数据库物化或平台写入。所有员工读取同一份全店数据；负责人/店铺仅作筛选，不限制看数范围。系统页通过独立 `/api/system` 合并经营快照与 root 定时发布的脱敏运行态，Portal 不能直接读取 Profile 或调用 systemd。
9. **写权限骨架**：未来写入必须同时满足全局开关、仓库授权身份、角色、能力和本人 `PRIMARY / SUPPORT` 店铺分配。员工可以看全部店，但不能把写权限带到别人负责的店。当前 HTTP mutation 与 SHEIN 写操作均拒绝。
10. **WebAPI 会话层**：正常经营数据读取使用按店 AES-256-GCM 加密的 Cookie 会话和固定同源 HTTP 客户端，不启动 Chrome；平台 `Set-Cookie` 原子轮换回密文。Profile 只用于首次建档和被恢复队列点名的单店，且必须先做页面身份回读、再做页面/HTTP 响应哈希一致性校验。会话失效只阻断该店，不阻断已经成功落库的 OpenAPI 域。订单管理会话同步复用同一会话层，且只允许 `order-management` 合同的固定 POST 查询路径。

## 门户层

门户保持原生 HTML / CSS / JavaScript，通过 hash 路由提供总控、发货订单、销量、
商品、库存供给、采购单、平台动态、自动化运营和系统健康
视图。总览先消费轻量 Dashboard 核心，再按当前筛选消费独立首页 API；
发货订单与采购单使用独立的服务端筛选与分页 API。它们只读取物化后的 JSON，Portal 不获得数据库
...
- 发货订单页是总控后的第一个业务页，按订单头与商品行展示急采/备货、状态、履约时效、仓库、发货单和收货进度；所有条件在服务端对全量索引筛选后再分页；
- 采购、交付、库存和缺货页消费独立只读事实，字段覆盖不足时显示未知；
- 平台页区分 Receiver / Worker 心跳、队列、订阅回读和规范化事件；
- 系统页把待处理问题、Profile 登记/续期双证据、核心任务、六域覆盖和安全边界按行动顺序展示，详细边界见 `docs/system-management.md`；
