...
7. **订单管理物化器**：与 Dashboard 物化器同一原子提升链路产出 `order-management.json`。固定九页合同（发货单、发货台、备货记录、运单、退货申请、退货列表、收货/退货异常、增值服务列表、质检报告）中，“发货单”与“运单”核心只从 `fact.purchase_order / fact.delivery` 单次可重复读快照物化；备货记录与运单明细在加密会话快照存在时按已验证的 `sso.geiwohuo.com` 固定 POST 合同合并（`/idms/order-apply/list`、`/clms/waybill/page`、`/clms/waybill/statistics`）。其余页面在原始证据不足以安全冻结请求前恒为 `UNAVAILABLE` 并写明原因，绝不猜路径；行内 `metrics / facts / details` 只含字段哈希校验过的脱敏 allowlist 文本/数字，地址、联系人、电话不落盘。分页/总数/去重/25 店覆盖任一失败，索引 `coverage` 非 `COMPLETE` 且 `promotable=false`，提升脚本只保留 staging 文件，不发布新索引。
