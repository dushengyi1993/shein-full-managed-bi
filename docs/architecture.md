...
2. **销量同步器**：完成稳定清单、每批 100 个 SKU 的销量抓取、日期锚定、合法零销量识别、不可变 raw/run/probe 回读和业务水位推进。非零无日期响应进入质量阻断。
3. **供应链同步器**：商品接口只补充属性；库存请求集合来自最新可接受销量清单。产品、库存 PI/VI/JI、缺货建议、采购和交付分别记录同步尝试，失败域不污染其他域。
