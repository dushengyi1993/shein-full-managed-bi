...
浏览器任务必须逐店关闭 Chrome，并保留 Profile 登录态。每日巡检发现的限时折扣缺口、
错价及高点击专项属于已长期授权的云端无人值守修复，必须保留固定写车道并执行
dry-run、计划/载荷 hash、事务式临时补量与恢复、逐组写后回读；资源不足时留待下一云端
写车道，不得改成 `deferred_to_local`。只有每周普通营销活动报名因全店浏览器工作量过大
时允许转本地浏览器；未授权的新普通活动仍不得自动提交。
...
| `browser-write` | 1 且全机独占 | 已授权限时折扣等浏览器写入 | 重任务门禁；每日修复固定云端车道，仅周普通活动可转本地 |
...
半托每日限时折扣 repair 固定使用 12、15、16、18、19、21 点的两段窗口：
`:15–:27` 与 `:45–:57`。每段只在精确队列仍有待办时启动；临近 `:27/:57` 不再派发
新组，已经进入真实事务的单组不中途杀，未完成队列留到下一段云端窗口。旧的
`:50 -> 下一小时 :17` 跨小时合同废止，避免撞到全托下一小时 `:02` 首页刷新。

为消除启动竞态，全托在上述六个小时的 `:10–:27`、`:40–:57` 不再启动新的
`browser-read/browser-write/db-heavy/io-heavy` 工作；已经在 `:02/:32` 启动的首页任务
由 11 分钟 systemd 上限保证最晚 `:13/:43` 收口，给半托写车道保留 2 分钟缓冲。
`api-light`、Webhook 与实时订单事实不让路，可与单个写浏览器并行。半托 Portal section
queue 在 repair 精确队列待处理时必须让出 `:14/:44`，低优先级缓存保留旧版本并延后。
...
上表之外，全托重任务还必须遵守“半托每日限时折扣写车道”的禁启窗口；这不会暂停
OpenAPI 销售、Webhook 或其它 `api-light` 快车道。
...
8. 在保留窗口用 `SHEIN_HOST_CLOCK_HHMM` 做门禁验收时，全托重车道返回
   `HALF_MARKETING_BROWSER_WRITE_RESERVED`，`api-light` 仍可启动。
