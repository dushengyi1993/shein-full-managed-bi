...
Webhook 入口只验签落库，异步 Worker 解密、标准化并写回查指令，不持有 OpenAPI
凭据。已落地的采购单和交付单适配器由独立 `sheinfm-supply` 进程按业务键精确回查，
只有 OpenAPI 返回对应行且仓库写入成功才完成指令。标准化事件在 30 秒内合并唤醒
Dashboard；定向回查以 path 触发，并在每小时 `:07/:17/:27/:37/:47/:57` 兜底。这条链路不等待日更或 Dashboard 大物化。
其他事件已立即进入“平台动态”，但在对应只读回查适配器上线前不冒充业务事实。
