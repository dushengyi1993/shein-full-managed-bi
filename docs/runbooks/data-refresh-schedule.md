...
| `04:45` | `shein-fm-home-finance-daily` | D-4 至 D-2 财务窗口滚动重读 |
| `05:45 / 06:45 / 07:45 / 09:45 / 10:45` | `shein-fm-home-daily` | 每批5店，重读 D-2/D-1；08点让给半托平台慢数据 |
| `11:50` | `shein-fm-home-daily-retry` | 只补前述分批留下的缺失范围 |
| `12:45` | `shein-fm-backup-archive` | 低优先级 COS 保留治理；在 13:20 ET 前硬停止 |

Dashboard 在每个批次终态触发独立只读物化：成功店的新事实、失败店的质量原因
和服务终态会一起进入页面。小时批次只要至少一家店完成实时事实写入就正常结束；
缺店、CDP 超时和登录态问题继续进入数据健康并由下一小时同批次重试。整批零成功
仍返回失败，不能把无数据伪装成刷新完成。独立物化 timer 只在偶数小时`:55`
兜底，不设开机触发，不再按“上次结束后两小时”漂移或每30分钟无条件重建。

## 平台结算门禁

每日首页任务在写入任何 D-1 事实前逐店调用官方 `get_update_time`。只有
`dataAnchorDate >= D-1` 才继续；否则记录 `HOME_SETTLEMENT_NOT_READY`，不把尚未
更新的空值或旧值冒充昨日最终数据。11:50 的重试仍执行同一门禁。
...
- 销量、供应链和财务任务共用 PostgreSQL advisory lock `8842137002`，不会并发
  消耗同一批 OpenAPI 应用额度。
- 日收口拆为5个固定五店批次，显式映射到
  `05/06/07/09/10:45`；错过的批次由11:50统一补漏。
- `:32–:43` 是永久首页核心车道。02:10 续期、03:45 供应链、04:45 财务、
  `:45` 日结和归档都设置硬超时，不允许跨入下一次 `:32`。
- 日常备份只负责生成和校验 dump；7日每日最新 + 3份额外备份的 COS 归档由
  12:45 独立任务执行，避免一次大文件复制拖住 02:20 半托会话链路。
- 历史回填和人工修复不得绕过上述两类锁，也不得与整点任务并发。
- 锁被占用时任务以受控跳过码 `75` 结束，不计为故障；后续整点或11:50补漏接管。
