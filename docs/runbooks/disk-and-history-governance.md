...
| 模式 | 触发方式 | 频率约束 |
| --- | --- | --- |
| `--mode daily` | `shein-fm-db-backup.timer`（每日 02:20 Asia/Shanghai） | 每个 **UTC 自然日**最多一次成功备份，重复触发直接跳过 |
...
- 生产默认在数据盘保留最近 **3 个 UTC 自然日**每天最新一份 dump；
- 再额外保留 **2 份最新**的其他 dump（应对同日部署和迁移的短尾）；
